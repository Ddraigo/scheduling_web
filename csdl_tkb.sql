...
-- bảng tb_PHAN_CONG (Phân công giảng dạy với tuần bắt đầu/kết thúc)
CREATE TABLE tb_PHAN_CONG (
    id      INT         IDENTITY(1,1) NOT NULL PRIMARY KEY,
    MaDot   VARCHAR(20) NOT NULL,
    MaLop   VARCHAR(12) NOT NULL,
    MaGV    VARCHAR(12) NULL,
	TuanBD  TINYINT,     -- Tuần bắt đầu dạy (1-15)
	TuanKT  TINYINT,     -- Tuần kết thúc dạy (1-15)
    UNIQUE (MaDot, MaLop),
    FOREIGN KEY (MaDot) REFERENCES tb_DOT_XEP(MaDot),
    FOREIGN KEY (MaLop) REFERENCES tb_LOP_MONHOC(MaLop),
    FOREIGN KEY (MaGV)  REFERENCES tb_GIANG_VIEN(MaGV),
	CHECK (TuanBD >= 1 AND TuanBD <= 15 AND TuanKT >= 1 AND TuanKT <= 15 AND TuanBD <= TuanKT)
);
GO
...
-- 1. Bảng ngày nghỉ cố định (gợi ý mặc định)
CREATE TABLE tb_NGAY_NGHI_CO_DINH (
    id INT IDENTITY(1,1) PRIMARY KEY,
    TenNgayNghi NVARCHAR(100) NOT NULL,
    Ngay CHAR(5) NOT NULL, -- Định dạng MM-DD, ví dụ '09-02' cho 2/9
    GhiChu NVARCHAR(200)
);

GO

INSERT INTO tb_NGAY_NGHI_CO_DINH (TenNgayNghi, Ngay, GhiChu)
VALUES
(N'Tết Dương lịch',                 '01-01', N'Nghỉ lễ theo luật'),
(N'Ngày Giải phóng miền Nam (30/4)', '04-30', N'Nghỉ lễ theo luật'),
(N'Quốc tế Lao động (1/5)',          '05-01', N'Nghỉ lễ theo luật'),
(N'Quốc khánh (2/9)',                '09-02', N'Nghỉ lễ theo luật');
GO

-- 2. Bảng ngày nghỉ áp dụng cho từng đợt (chỉ những ngày này mới thực sự được nghỉ)
CREATE TABLE tb_NGAY_NGHI_DOT (
    id INT IDENTITY(1,1) PRIMARY KEY,
    MaDot VARCHAR(20) NOT NULL,
    NgayBD DATE NOT NULL,        -- Ngày bắt đầu nghỉ
    SoNgayNghi INT NOT NULL,     -- Số ngày nghỉ
    TuanBD INT,                  -- Tuần bắt đầu (tuỳ chọn, để tham khảo)
    TuanKT INT,                  -- Tuần kết thúc (tuỳ chọn, để tham khảo)
    TenNgayNghi NVARCHAR(100),
    GhiChu NVARCHAR(200),
    UNIQUE (MaDot, NgayBD, SoNgayNghi),
    FOREIGN KEY (MaDot) REFERENCES tb_DOT_XEP(MaDot),
	CHECK (SoNgayNghi > 0)
);
...
/* ===== Thời khoá biểu - Lưu lịch dạy cho mỗi buổi học ===== */
CREATE TABLE tb_TKB(
    MaTKB       VARCHAR(15)  NOT NULL PRIMARY KEY, 
	MaDot       VARCHAR(20) NOT NULL,
	MaLop		VARCHAR(12) NOT NULL,
    MaPhong     VARCHAR(12)  NULL,                  -- Chỉ xác định nếu đã sắp xếp phòng
    TimeSlotID  VARCHAR(10)  NOT NULL,              -- Thứ + Ca
    TuanHoc     VARCHAR(64) NOT NULL,              -- Pattern tuần học (VD: "1111111000000000")
	NgayBD		DATE NOT NULL,                      -- Ngày bắt đầu buổi học cụ thể
	NgayKT		DATE NOT NULL,                      -- Ngày kết thúc buổi học cụ thể (thường = NgayBD)
    NgayTao     DATETIME2 DEFAULT SYSUTCDATETIME(),
    GhiChu      NVARCHAR(200) NULL,

	-- Constraint duy nhất: 1 lớp, 1 phòng, 1 TimeSlot trong 1 đợt → 1 buổi học
...
-- Indexes cho PhanCong (query phân công theo GV, Lớp, Đợt)
...
CREATE INDEX IX_PC_MaDot_MaGV  ON tb_PHAN_CONG(MaDot, MaGV);  -- Query lịch GV theo đợt

-- Indexes cho TKB (query lịch theo đợt, lớp, phòng, slot)
...
CREATE INDEX IX_TKB_NgayBD     ON tb_TKB(NgayBD);  -- Query lịch theo ngày
CREATE INDEX IX_TKB_MaDot_MaLop ON tb_TKB(MaDot, MaLop);

-- Indexes cho NgayNghiDot
CREATE INDEX IX_NGAYNGHI_MaDot ON tb_NGAY_NGHI_DOT(MaDot);

-- Indexes cho LopMonHoc
CREATE INDEX IX_LMH_MaMonHoc   ON tb_LOP_MONHOC(MaMonHoc);

-- Indexes cho DotXep
CREATE INDEX IX_DOT_MaDuKienDT ON tb_DOT_XEP(MaDuKienDT);
...
CREATE PROCEDURE dbo.sp_AddWeekBreaks_Week1_Week9_UsingDuKien
    @MaDot VARCHAR(20) = NULL     -- NULL = áp cho tất cả đợt
AS
BEGIN
    SET NOCOUNT ON;

    /* 1) DOT: khung ngày của đợt lấy từ DUKIEN_DT */
    IF OBJECT_ID('tempdb..#DOT') IS NOT NULL DROP TABLE #DOT;
    SELECT dx.MaDot, k.NgayBD, k.NgayKT
    INTO #DOT
    FROM dbo.tb_DOT_XEP dx
    JOIN dbo.tb_DUKIEN_DT k ON k.MaDuKienDT = dx.MaDuKienDT
    WHERE (@MaDot IS NULL OR dx.MaDot = @MaDot)
      AND k.NgayBD IS NOT NULL AND k.NgayKT IS NOT NULL
      AND k.NgayBD > '1900-01-01' AND k.NgayKT > '1900-01-01';

    /* 2) W: tuần 1 và tuần 9 (mỗi tuần 7 ngày) */
    IF OBJECT_ID('tempdb..#W') IS NOT NULL DROP TABLE #W;
    SELECT MaDot, 1 AS WeekNo, NgayBD AS StartDate
    INTO #W
    FROM #DOT
    UNION ALL
    SELECT MaDot, 9 AS WeekNo, DATEADD(DAY, 8*7, NgayBD)
    FROM #DOT;

    /* 3) UPDATE các hàng đã có (đúng ngày & 7 ngày) -> TuanBD=TuanKT=WeekNo */
    UPDATE t
    SET  t.TuanBD      = w.WeekNo,
         t.TuanKT      = w.WeekNo,
         -- KHÔNG ép SoNgayNghi=7 để tránh đụng UNIQUE nếu có bản ghi trùng
         t.TenNgayNghi = N'Nghỉ tuần ' + CAST(w.WeekNo AS NVARCHAR(10)),
         t.GhiChu      = COALESCE(NULLIF(t.GhiChu, N''), N'Tự động thêm (nghỉ 1 tuần)')
    FROM dbo.tb_NGAY_NGHI_DOT t
    JOIN #W   w ON w.MaDot = t.MaDot AND t.NgayBD = w.StartDate
    JOIN #DOT d ON d.MaDot = t.MaDot
    WHERE (t.TuanKT IS NULL OR t.TuanKT = 0 OR t.TuanBD IS NULL)
      AND ISNULL(t.SoNgayNghi,7) = 7
      AND w.StartDate BETWEEN d.NgayBD AND d.NgayKT
      AND DATEADD(DAY, 6, w.StartDate) <= d.NgayKT;   -- tuần nằm trọn trong kỳ

    /* 4) INSERT những hàng chưa có */
    INSERT INTO dbo.tb_NGAY_NGHI_DOT
        (MaDot, NgayBD, SoNgayNghi, TuanBD, TuanKT, TenNgayNghi, GhiChu)
    SELECT w.MaDot,
           w.StartDate,
           7,
           w.WeekNo,                  -- TuanBD
           w.WeekNo,                  -- TuanKT = TuanBD
           N'Nghỉ tuần ' + CAST(w.WeekNo AS NVARCHAR(10)),
           N'Nghỉ tuần thi'
    FROM #W w
    JOIN #DOT d ON d.MaDot = w.MaDot
    LEFT JOIN dbo.tb_NGAY_NGHI_DOT t
           ON t.MaDot = w.MaDot
          AND t.NgayBD = w.StartDate
          AND ISNULL(t.SoNgayNghi,7) = 7
    WHERE w.StartDate BETWEEN d.NgayBD AND d.NgayKT
      AND DATEADD(DAY, 6, w.StartDate) <= d.NgayKT
      AND t.id IS NULL;              -- chỉ chèn nếu chưa có
END
GO

-- Chạy cho tất cả đợt:
EXEC dbo.sp_AddWeekBreaks_Week1_Week9_UsingDuKien @MaDot = 'DOT1_2025-2026_HK1';

select * from tb_NGAY_NGHI_DOT
select * from tb_RANG_BUOC_MEM